...
tagRouter.get('/all', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT nimi FROM Tagi')
    await res.json(rows)
  } catch (exception) {
    res.send(exception)
  }
...
tagRouter.get('/directory/:tag_name', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query('SELECT note.id,title,content FROM note LEFT JOIN notetag ON notetag.note_id = note.id LEFT JOIN tag ON tag.id = notetag.tag_id WHERE tag.name =($1) GROUP BY note.id,note.title,note.content', [tag])
    await res.json(rows)
  } catch (exception) {
...
tagRouter.post('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query('INSERT INTO tag(name) VALUES ($1) RETURNING name', [tagName])
    await res.json(rows)
  } catch (exception) {
...
tagRouter.post('/notetag/add', async (req, res) => {
  ...
  try {
    await pool.query('INSERT INTO notetag(note_id, tag_id) VALUES($1, $2)', [req.body.note_id, req.body.tag_id])
