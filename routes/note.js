...
noteRouter.post('/', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query('INSERT INTO note(title,content,account_id) VALUES($1, $2, $3) RETURNING id', [body.title, body.content, acc_id])
      id = await rows[0].id
...
noteRouter.put('/note/:id', async (req, res) => {
  ...
  try {
    ...
    if (!token || !decodedToken.id) {
      ...
      try {
        client.query('BEGIN')
        await client.query('UPDATE note SET title =($1), content =($2) WHERE note.id =($3) AND account_id =($4)', [body.title, body.content, id, acc_id])
...
noteRouter.delete('/note/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query('BEGIN')
    const { rows } = await client.query('SELECT * FROM note WHERE id = ($1) AND account_id = ($2)', [id, acc_id])
    if (rows[0] !== undefined) {
